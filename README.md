...
Keep using `http://127.0.0.1:8787/` in the Mac browser for the local admin UI.
...
The admin UI, token APIs, and raw activity log accept loopback connections
only. `/mcp` accepts remote clients with a bearer token. If a client uses a
different hostname, add it to `DD_MCP_ALLOWED_HOSTS`.
...
| `DD_MCP_ALLOWED_HOSTS` | local hostnames | DNS-rebinding allowlist |
